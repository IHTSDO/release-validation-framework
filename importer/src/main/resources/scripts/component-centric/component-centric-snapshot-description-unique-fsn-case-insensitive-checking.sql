/******************************************************************************** 
	...
	The reason to create fsn_new and active_fsns temp tables due to the collation of term in the description tables
	is in binary which is case sensitive but for checking FSN uniquenss it requires case insensitive. 
********************************************************************************/
drop table if exists fsn_new;
create table fsn_new (conceptid bigint(20), term varchar(255),key idx_fsn_new(term)) default charset=utf8;

insert into fsn_new(conceptid,term) (select distinct b.conceptid,b.term from curr_concept_s a join curr_description_d b on a.id=b.conceptid
...
insert into qa_result (runid, assertionuuid, concept_id, details)
	...
	from fsn_new a where exists (select count(*) total from active_fsns b where a.term = b.term having total >1);
