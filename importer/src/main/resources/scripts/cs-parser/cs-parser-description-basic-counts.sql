...
/******************************************************************************** 
	cs-parser-description-basic-counts
...
********************************************************************************/
	...
	left join prev_description_s b on a.allidcount = b.id
	where b.id is null;
	...
	select 
		...
		'<ASSERTIONUUID>',
		'<ASSERTIONTEXT>',
		concat('The number of mapped distinct CommitTime-Ids to distinct Ids from the description changeset file is incorrect') 	
	...
	select 
		...
		'<ASSERTIONUUID>',
		'<ASSERTIONTEXT>',
		concat('The Sum of new and existing Ids to distinct Ids from the description changeset file is incorrect') 	
