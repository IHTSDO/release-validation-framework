...
/******************************************************************************** 
	cs-parser-relationship-basic-counts
...
********************************************************************************/
	...
	select count(distinct(a.id)) as allidcount from cs_relationship a;
	
	
	-- Count of Ids that existed in previous release
	create view v_existingid as
	select count(*) as existingidcount 
	from v_allid a, prev_stated_relationship_s b
	...
	left join prev_stated_relationship_s b on a.allidcount = b.id
	where b.id is null;
	...
	select 
		...
		'<ASSERTIONUUID>',
		'<ASSERTIONTEXT>',
		concat('The number of mapped distinct CommitTime-Ids to distinct Ids from the relationship changeset file is incorrect') 	
	...
	select 
		...
		'<ASSERTIONUUID>',
		'<ASSERTIONTEXT>',
		concat('The Sum of new and existing Ids to distinct Ids from the relationship changeset file is incorrect') 	
