...
/* in the delta; not in the full */
	...
	'<ASSERTIONUUID>',
	'<ASSERTIONTEXT>',
	concat('Stated relationship: id=',a.id, ' is in current full file, but not in prior full file.') 	
	...
	select 
		...
		'<ASSERTIONUUID>',
		'<ASSERTIONTEXT>',
		concat('Stated relationship: id=',a.id, ' is in FULL file, but not in delta file.') 
	from curr_stated_relationship_f a
	left join prev_stated_relationship_f b
		...
		and a.characteristictypeid = b.characteristictypeid
		and a.modifierid = b.modifierid
	left join curr_stated_relationship_d c
		on a.id = c.id
		and a.effectivetime = c.effectivetime
		and a.active = c.active
		and a.moduleid = c.moduleid
