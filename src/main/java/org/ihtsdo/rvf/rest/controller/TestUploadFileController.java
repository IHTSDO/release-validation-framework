...
public class TestUploadFileController {
	...
	public ResponseEntity uploadPostTestPackage(
			@Parameter(name = "RF2 release file in zip format") @RequestParam(value = "file") final MultipartFile file,
			@Parameter(name = "Defaults to false when not provided") @RequestParam(value = WRITE_SUCCESSES, required = false) final boolean writeSucceses,
			@Parameter(name = "manifest.xml file") @RequestParam(value = MANIFEST, required = false) final MultipartFile manifestFile,
			final HttpServletResponse response) throws IOException {
		// load the filename
		final String filename = file.getOriginalFilename();
		if (!filename.endsWith(ZIP)) {
	...
	public ResponseEntity<Map<String, String>> runPostTestPackage(
			...
			@Parameter(name = "Default to false to reduce the size of report file") @RequestParam(value = WRITE_SUCCESSES, required = false, defaultValue = "false") final boolean writeSucceses,
			...
			@Parameter(name = "Required for extension release testing") @RequestParam(value = PREVIOUS_DEPENDENCY_EFFECTIVE_TIME, required = false) final String previousDependencyEffectiveTime,
			@Parameter(name = "Unique number e.g Timestamp") @RequestParam(value = RUN_ID) final Long runId,
			...
			@Parameter(name = "Default module ID of components in the MS extension") @RequestParam(value = DEFAULT_MODULE_ID, required = false) final String defaultModuleId,
			@Parameter(name = "Module IDs of components in the MS extension. Used for filtering results in Drools validation. Values are separated by comma") @RequestParam(value = INCLUDED_MODULES, required = false) final String includedModules,
			...
			@Parameter(name = "Enable change not at task level validation. This parameter needs to be combined together with enableTraceabilityValidation.") @RequestParam(value = ENABLE_CHANGE_NOT_AT_TASK_LEVEL_VALIDATION, required = false, defaultValue = "false") final boolean enableChangeNotAtTaskLevelValidation,
			...
			@Parameter(name = "Refset Descriptor members are being excluded, used for traceability check.") @RequestParam(value = EXCLUDED_REFSET_DESCRIPTOR_MEMBERS, required = false) final String excludedRefsetDescriptorMembers,
			@Parameter(name = "Base timestamp of content branch, used for traceability rebase changes summary report.") @RequestParam(required = false) final Long contentBaseTimestamp,
			@Parameter(name = "Head timestamp of content branch, used for stale state detection.") @RequestParam(required = false) final Long contentHeadTimestamp,
			@Parameter(name = "Name of the response queue.") @RequestParam(value = RESPONSE_QUEUE, required = false) final String responseQueue,
			@Parameter(name = "User name.") @RequestParam(value = USER_NAME, required = false) final String username,
			@Parameter(name = "Authentication token.") @RequestParam(value = AUTH_TOKEN, required = false) final String authenticationToken,
	...
	public ResponseEntity<Map<String, String>> runPostTestPackageViaS3(
			...
			@Parameter(name = "True if the test file contains RF2 delta files only. Defaults to false.") @RequestParam(value = RF2_DELTA_ONLY, required = false, defaultValue = "false") final boolean isRf2DeltaOnly,
			@Parameter(name = "Defaults to false to reduce the size of report file") @RequestParam(value = WRITE_SUCCESSES, required = false) final boolean writeSucceses,
			@Parameter(name = "manifest.xml file path in AWS S3") @RequestParam(name =  "manifestFileS3Path", required = false) final String manifestFileS3Path,
			@Parameter(name = "Assertion group names") @RequestParam(value = GROUPS) final List<String> groupsList,
			@Parameter(name = "Drools rules group names") @RequestParam(value = DROOLS_RULES_GROUPS, required = false) final List<String> droolsRulesGroupsList,
			@Parameter(name = "Required for non-first time international release testing") @RequestParam(value = PREVIOUS_RELEASE, required = false) final String previousRelease,
			@Parameter(name = "Required for extension release testing") @RequestParam(value = DEPENDENCY_RELEASE, required = false) final String extensionDependency,
			@Parameter(name = "Required for extension release testing") @RequestParam(value = PREVIOUS_DEPENDENCY_EFFECTIVE_TIME, required = false) final String previousDependencyEffectiveTime,
			@Parameter(name = "Unique run id e.g Timestamp") @RequestParam(value = RUN_ID) final Long runId,
			@Parameter(name = "Defaults to 10 when not set") @RequestParam(value = FAILURE_EXPORT_MAX, required = false, defaultValue = "10") final Integer exportMax,
			@Parameter(name = "The sub folder for validation reports") @RequestParam(value = STORAGE_LOCATION) final String storageLocation,
			@Parameter(name = "Defaults to false") @RequestParam(value = ENABLE_DROOLS, required = false) final boolean enableDrools,
			@Parameter(name = "Effective time, optionally used in Drools validation, required if Jira creation flag is true") @RequestParam(value = EFFECTIVE_TIME, required = false) final String effectiveTime,
	...
	public ResponseEntity uploadPreTestPackage(
			@Parameter(name = "RF2 input file prefixed with rel2", required = true) @RequestParam(value = "file") final MultipartFile file,
