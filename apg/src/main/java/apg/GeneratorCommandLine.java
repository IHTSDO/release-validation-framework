...
class GeneratorCommandLine {
	...
	enum Params{
        ...
        String prefix(){return p;}
        String defaultValue(){return v;}
        String description(){return d;}
	}
		...
		try{
			...
			for(String tok : args){
				...
				// test all parameters
					...
					inputFiles.add(paramValues[Params.INPUT.ordinal()]);
				}
...
//			}
	...
	private boolean isHelp(String tok){
		boolean ret = false;
		// help == ? or /help
		if('?' == tok.charAt(0)){ret = true;}
		else if(tok.equalsIgnoreCase("/help")){ret = true;}
		return ret;
	}
