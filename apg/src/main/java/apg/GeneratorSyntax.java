...
//	}
	...
	static class SyntaxOpcode {
		...
		@Override public String toString(){
			...
			case REP:
				...
				buf.append(min);
				buf.append(", ");
				if(max == Integer.MAX_VALUE){buf.append("inf");}
				else{buf.append(max);}
				buf.append("], ");
				printChildren(buf, childOpcodes);
				buf.append(")\n");
	...
	private static void logError(int offset, String msg){
		...
		if(line == null){errors.add("line[undefined]: " + msg);}
		else{errors.add("line[" + line.lineno + "]: " + msg);}
	}
...
//	}
	...
	private static int decDigits(char[] string, int offset, int length){
		...
		for(int i = offset; i < max; i++){value = 10 * value + (int) (string[i] - 48);}
		return value;
	}
	...
	private static int hexDigits(char[] string, int offset, int length){
		...
		for(int i = offset; i < max; i++){
			...
			if(digit < 58){digit = digit - 48;}
			else if(digit < 71){digit = digit - 65 + 10;}
			else{digit = digit - 97 + 10;}
	...
	private static int binaryDigits(char[] string, int offset, int length){
		int value = 0;
		int max = offset + length;
		for(int i = offset; i < max; i++){value = 2 * value + (int) (string[i] - 48);}
		return value;
	}
...
//			}
	...
	static class GenRule extends RuleCallback {
		...
		@Override public int preBranch(int offset){
			...
			LineCatalog.Line line = lineCatalog.getLineFromOffset(offset);
			if(line != null){currentRule.lineno = line.lineno;}
			return -1;
		}
		@Override public int postBranch(int offset, int length){
	...
	static class Udt extends RuleCallback {
		...
		@Override public int postBranch(int offset, int length){
			if(length >= 0){
				...
				if(thisPhrase.charAt(0) == 'e'){thisOpcode.mayBeEmpty = true;}
				else{thisOpcode.mayBeEmpty = false;}
	...
	static class Rep extends RuleCallback {
		...
		@Override public int postBranch(int offset, int length){
			if(length >= 0){
				...
				if(thisOpcode.min == -1){thisOpcode.min = 0;}
				if(thisOpcode.max == -1){thisOpcode.max = Integer.MAX_VALUE;}
			}
			return -1;
		}
	}
	...
	static class Tbs extends RuleCallback {
		...
		@Override public int postBranch(int offset, int length){
			if(length >= 0){
				...
				int i = 0;
				for(char c : tbs){thisOpcode.string[i++] = c;}
				tbs.clear();
				addChildToParent(parentStack, thisOpcode);
			}
			return -1;
	...
	static class Trg extends RuleCallback {
		...
		@Override public int postBranch(int offset, int length){
			if(length >= 0){
				...
				if(tbs.size() == 2){thisOpcode.max = (int)tbs.elementAt(1);}
				else{thisOpcode.max = thisOpcode.min;}
				tbs.clear();
				addChildToParent(parentStack, thisOpcode);
			}
			return -1;
		...
		for(SyntaxRule rule : rules){
			for(int i = rule.opcodeOffset; i < (rule.opcodeOffset + rule.opcodeCount); i++){
				...
				case UDT:
					...
					if(entry != null){entry.count = entry.count + 1;}
					else{logError(opcode.index, "UDT name: " + opcode.name + " referenced but not defined");}
					break;
				case REP:
					ret.rep++;
					break;
				case RNM:
					...
					if(entry != null){entry.count = entry.count + 1;}
					else{logError(opcode.index, "RNM name: " + opcode.name + " referenced but not defined");}
					break;
				}
			}
		}
