	...
	public void runTraceabilityComparison(ValidationStatusReport report, ValidationRunConfig validationConfig) {
		...
		try {
			...
			if (summaryReport.getChangesNotAtTaskLevel() != null) {
				Set<ConceptChange> allConceptChanges = summaryReport.getChangesNotAtTaskLevel().stream().flatMap(activity -> activity.getConceptChanges().stream()).collect(Collectors.toSet());
				Set<ComponentChange> allComponentChanges = allConceptChanges.stream().flatMap(component -> component.getComponentChanges().stream()).collect(Collectors.collectingAndThen(Collectors.toCollection(() -> new TreeSet<>(Comparator.comparing(ComponentChange::getComponentId))), HashSet::new));
				for(ComponentChange componentChange : allComponentChanges) {
					if (remainingFailureExport.get() > 0) {
						remainingFailureExport.decrementAndGet();
						changesNotAtTaskLevel.addFirstNInstance(new FailureDetail(traceabilityComponentIdToConceptIdMap.get(componentChange.getComponentId()), String.format("%s change for %s has been made on a branch that was not task level.", componentChange.getComponentType(), componentChange.getComponentId())).setComponentId(componentChange.getComponentId()));
					}
				}
				changesNotAtTaskLevel.setFailureCount(Long.valueOf(allComponentChanges.size()));
			}
